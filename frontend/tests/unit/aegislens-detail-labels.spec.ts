...
describe('AegistonLens detail display labels', () => {
  it('uses AegistonLens branding throughout the legallens detail view without changing its route slug', () => {
    ...
    expect(detailPage).toContain("title={slug === 'legallens' ? `${displayName} 合约智审` : `${displayName} ${product.nameCn}`}");
