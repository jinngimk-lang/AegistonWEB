...
| **第 10 条 · 全站移除 `.utility-bar`，将「中文 / EN」迁入白色主导航检索与联系按钮区域（2026-09-09）** | 业主按官网实际截图明确要求移除深蓝顶栏，并把语言状态迁到白色主导航中、位于检索区域与「联系我们」之间。保留中文当前态与 EN 的 `aria-disabled` /「英文站建设中」语义；主菜单、检索、联系按钮与内容数据均不改。该条覆盖 §3.1 / §3.2 与 A-5 中“继续渲染 utility-bar”的历史基线。 |
| **第 11 条 · 首页移除 `.hero-eyebrow` 的「智瞳安宇 · AEGISTON」重复品牌标签（2026-09-09）** | 白色主导航左侧已经完整展示品牌标识，首页 Hero 再重复一次品牌标签形成冗余。按业主截图只移除 eyebrow 本身，Hero 标题、副标题、CTA、背景、遮罩和其余度量保持不变。 |
| **第 12 条 · 全站页脚隐藏「公司定位」与「网站地图」入口（2026-09-21）** | 业主明确要求首页及所有内页的全站页脚不再展示这两个入口。仅隐藏页脚入口，对应页面与路由本身保留；其它页脚链接、栏目、样式、导航与页面内容均不改。 |
